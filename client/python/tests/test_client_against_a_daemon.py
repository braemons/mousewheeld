...
What `test_convert_seam.py` cannot check: that the rpcs are the ones the daemon
...
def test_it_answers_who_it_is_without_a_board(wheel):
...
def test_the_simulated_board_is_a_board(wheel):
    device = wheel.device()
...
def test_the_wheel_turns(wheel):
    first = wheel.state().axis("wheel")
    assert not wheel.state().health.stale
    # The simulator runs; counts are cumulative, so this only ever grows.
    assert wheel.state().axis("wheel").counts >= first.counts


def test_an_unknown_axis_is_a_key_error_rather_than_a_default(wheel):
    with pytest.raises(KeyError, match="no axis named ball"):
        wheel.state().axis("ball")


def test_zeroing_moves_the_api_origin(wheel):
    after = wheel.zero_position()
    assert abs(after.axis("wheel").position_cm) < 1.0


def test_the_stream_is_decimated_and_says_so(wheel):
    ...
    frames = wheel.watch_state(rate_hz=5)
    samples = []
    for frame in frames:
...
def test_a_stored_set_reads_back_as_types(wheel):
    assert "goal" in wheel.zone_sets()
    zone = wheel.zone_set("goal").zones[0]
    assert zone.min_cm == (ZoneBoundReference("goal_cm"),)
...
def test_the_same_set_reads_back_as_the_file_it_is(wheel):
    ...
    assert wheel.check_zone_set_file(file.text).ok


def test_a_file_that_does_not_parse_is_refused_with_where(wheel):
    with pytest.raises(DaemonRefusedTheRequest) as refusal:
        wheel.check_zone_set_file('{"zone_set_version": 1, "zones": [{"nam": "goal"}]}')
...
def test_a_parameterised_set_names_what_it_needs(wheel):
    report = wheel.check_zone_set("goal")
    assert report.ok
...
def test_a_missing_set_is_not_found_and_says_which(wheel):
    with pytest.raises(DaemonRefusedTheRequest) as refusal:
        wheel.zone_set("corridor")
    assert refusal.value.status == "not_found"
    assert refusal.value.error == "no_such_zone_set"
    assert refusal.value.context == "corridor"


def test_a_name_that_is_a_path_is_refused_as_a_name(wheel):
    """`../../etc/passwd` is a refusal naming the rule it broke, not a
    traversal."""
    with pytest.raises(DaemonRefusedTheRequest) as refusal:
        wheel.zone_set("../../etc/passwd")
    assert refusal.value.error == "bad_zone_set_name"


def test_the_two_dimensional_ball_is_unimplemented_rather_than_absent(wheel):
...
def test_applying_a_measurement_nobody_took_is_refused(wheel):
    with pytest.raises(DaemonRefusedTheRequest) as refusal:
        wheel.apply_measurement()
    assert refusal.value.status == "failed_precondition"
    assert refusal.value.error == "nothing_measured"


def test_finishing_a_measurement_nothing_moved_for_is_refused(wheel):
    """A wheel that did not turn is not a calibration of zero counts per
    centimetre; it is a measurement that did not happen."""
    wheel.start_measuring("wheel", 100.0)
    with pytest.raises(DaemonRefusedTheRequest) as refusal:
        wheel.finish_measuring()
    assert refusal.value.error == "too_few_counts"


def test_a_measurement_reports_against_what_is_configured(wheel):
    started = wheel.start_measuring("wheel", 100.0)
    ...
    while wheel.state().axis("wheel").counts - started.counts_at_start < 1000:
        if time.monotonic() > deadline:
            pytest.fail("the simulated wheel never turned")
        time.sleep(0.05)

    result = wheel.finish_measuring()
...
def test_the_config_says_whether_the_segment_is_open(wheel):
    config = wheel.config()
...
def test_a_config_patch_changes_only_what_it_names(wheel):
    before = wheel.config()
    after = wheel.set_config(ring_minutes=before.ring_minutes + 1)
    assert after.ring_minutes == before.ring_minutes + 1
    assert after.rate_hz == before.rate_hz
    wheel.set_config(ring_minutes=before.ring_minutes)


def test_the_lines_come_from_the_rig_config(wheel):
    lines = wheel.lines()
    assert lines
    assert any(line.name == "zone_goal" for line in lines)


def test_the_wire_log_holds_the_conversation(wheel):
    lines = wheel.wire_log()
...
def test_a_daemon_that_is_not_there_is_not_connected():
    ...
    with MousewheeldClient("127.0.0.1:1") as absent, pytest.raises(DaemonOrBoardIsUnavailable) as refusal:
        absent.state()
